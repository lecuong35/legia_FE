<template>
    <div class="w-full p-[10px] mobile:mt-[50px]
    border rounded-lg" v-if="loaded">
        ...
        <div class="w-full pt-[10px]">
                    ...
                    <tr v-for="bill in bills" :key="bill.id">
                        ...
                        <td>
                            <p class="whitespace-nowrap">
                                {{bill.products[0].name.slice(0,15)}}...
                                <span class="text-[10px]">
                                   x {{ bill.cart_items[0].quantity }}
                                </span>
                            </p>
            ...
            <!-- no bills -->
            <div class="w-full my-[20px]" v-else>
...
<script>
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            onMounted(() => {
                store.dispatch('bill/userGetBills', [status_bills.value, 1])
                .then(() =>{
                    var response = [];
                    ...
                    if (store.getters['bill/errors'].length > 0) no_record.value = true;
            ...
            const hidden_modal = (id) => {
                router.push({path: '../don-hang/' + id});
            };

            watch(() => store.getters['bill/recent_pending_bills'], () => {
                bills.value = store.getters['bill/recent_pending_bills'].data;
            })

            const cancel_bill = async (id) => {
                if (confirm('Bạn có muốn hủy đơn này không ?')) {
                    if (ls.get('token_user') != null) {
                        await store.dispatch('bill/userDeleteBill', id);
                    }
                    else {
                        await store.dispatch('bill/handleBill', [id, 'CANCELLED']);
                    }
                }
            };
            ...
            const set_page = async (page) => {
                ...
                var  response = store.getters['bill/recent_bills'];
                bills.value = response.data;
                list_page.value = response.current_page;
            };
